...
if (!existsSync(sitemapFile)) {
  console.error(
    '✗ .output/public/sitemap.xml is missing — server/routes/sitemap.xml.ts did not prerender'
  )
  process.exit(1)
}
...
if (missing.length || extra.length) {
  fail(
    1,
    'the generated sitemap and the prerendered canonicals disagree',
    [
...
const undated = [...sitemapEntries].filter(
  ([, entry]) => !/^\d{4}-\d{2}-\d{2}$/.test(entry.lastmod ?? '')
)
if (undated.length) {
...
for (const name of ['privacy', 'terms']) {
  ...
  if (!want.length) {
    fail(4, `${first} rendered no sections`, [
      'the anchor ids the table of contents links to are gone'
    ])
    continue
  }
  const duplicates = want.filter((id, index) => want.indexOf(id) !== index)
  if (duplicates.length) {
    fail(4, `${first} repeats a section id`, [
      show(new Set(duplicates)),
      'ids are anchor targets and must be unique'
    ])
  }
...
for (const page of indexable) {
  ...
  if (!webPage) {
    fail(5, `${page.route} has no WebPage node`, [
