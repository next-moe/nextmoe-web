...
const collections = Object.keys({ ...manifest.dependencies, ...manifest.devDependencies })
  .filter((name) => name.startsWith('@iconify-json/'))
  .map((name) => name.slice('@iconify-json/'.length))

if (!collections.length) {
  console.error('✗ gate: no @iconify-json/* package installed — every check below is vacuously true')
  process.exit(1)
}
...
const catalogues = Object.fromEntries(
  collections.map((collection) => [
    collection,
    JSON.parse(readFileSync(join(root, 'node_modules', '@iconify-json', collection, 'icons.json'), 'utf8'))
  ])
)
...
const used = new Map()
for (const path of sourceDirs.flatMap(walk).filter((path) => ['.vue', '.ts'].includes(extname(path)))) {
...
console.log(`→ gate 1/2: bundle list matches the icons the code renders (${collections.join(' / ')})`)
...
if (extra.length) {
  fail(1, 'ICON_NAMES entries no call site renders', [show(extra), 'they inflate the client bundle for nothing'])
}
...
for (const name of ICON_NAMES) {
  ...
  if (!catalogue.icons[icon] && !catalogue.aliases?.[icon]) {
    fail(2, `${name} does not exist in @iconify-json/${collection}`, ['a typo here is silent at build time'])
  }
}
